...
class BCELoss(nn.Module):
    def __init__(self):
        super().__init__()
        # 1 通道用 BCEWithLogitsLoss，2 通道用 CE
        self.bce1 = nn.BCEWithLogitsLoss()
        self.ce2  = nn.CrossEntropyLoss()

    def forward(self, pred, target):
        """
        pred:   [B,C,H,W]  C=1 or 2
        target: [B,H,W]    0/1  long
        """
        if pred.size(1) == 1:
            # 1 通道：target 转 float
            return self.bce1(pred.squeeze(1), target.float())
        else:
            # 2 通道：直接 CE
            return self.ce2(pred, target)
...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        dice_score = (2 * intersection.sum(1) + smooth) / \
                     (prob_.sum(1) + target_.sum(1) + smooth)
...
class GT_BceDiceLoss_Mine(nn.Module):
    def __init__(self, wb=1, wd=1):
        super(GT_BceDiceLoss_Mine, self).__init__()
        self.bcedice = BceDiceLoss(1, 2)
        self.bcedice2 = BceDiceLoss(0.5, 1)
    ...
    def forward(self, gt_pre, key_points, out, target, points):
        ...
        gt_loss = self.bcedice2(gt_pre4, target) * 0.1 + self.bcedice2(gt_pre3, target) * 0.2 + self.bcedice2(gt_pre2, target) * 0.3 + self.bcedice2(gt_pre1, target) * 0.4 
        return bcediceloss + gt_loss + boundary_loss
